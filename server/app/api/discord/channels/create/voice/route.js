...
/**
 *  Creates a voice channel in a Discord server.
...
const createTextChannel = async (botToken, guild, chanName) => {
    try {
        const chanExists = await channelExists(DISCORD_TOKEN, guild, chanName, 2);
        if (chanExists === false) {
...
export const GET = async (req) => {
    try {
        ...
        return Response.json(`The voice channel ${chanName} has been created successfully`);
    } catch (error) {
